                                        ...
                                        size_t count, unsigned int timeout_ms)
{
	...
	while (bytes_written < count) {
		/* Check timeout only if we have run select() at least once, to
		 * avoid any issues if a short timeout is reached before
		 * select() is even run. */
                              ...
                              unsigned int timeout_ms)
{
	...
	while (1) {
		/* Check timeout only if we have run poll() at least once, to
		 * avoid any issues if a short timeout is reached before
		 * poll() is even run. */
		if (timeout_ms && started) {
			gettimeofday(&now, NULL);
